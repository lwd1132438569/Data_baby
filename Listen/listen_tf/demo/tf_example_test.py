...
def conv1d_layer(input_tensor, size, dim, activation, scale, bias):
    ...
    with tf.variable_scope("conv1d_" + str(conv1d_index)):
        ...
        if not bias:
            ...
            m, v = tf.cond(tf.Variable(False, trainable=False), update_running_stat,lambda: (mean_running, variance_running))  # 对于tf.conf的理解：http://blog.csdn.net/m0_37041325/article/details/76908660
            # 各个网络层输出的归一化
