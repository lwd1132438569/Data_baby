...
org_train_file = 'C:\dataSet\pos&neg_commit\\training.1600000.processed.noemoticon.csv'
org_test_file = 'C:\dataSet\pos&neg_commit\\testdata.manual.2009.06.14.csv'
    ...
    with open(org_file, buffering=10000, encoding='latin-1') as f:
        try:
            for line in f:  # "4","2193601966","Tue Jun 16 08:40:49 PDT 2009","NO_QUERY","AmandaMarie1028","Just woke up. Having no school is the best feeling ever "
                line = line.replace('"', '')   #把字符串中的双引号替换成空格
                clf = line.split(',')[0]  # 4
                if clf == '0':
                    clf = [0, 0, 1]  # 消极评论
                elif clf == '2':
